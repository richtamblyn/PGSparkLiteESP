...
bool update_spark_state()
{
  ...
  {
    if (preset_received == true || millis() - sync_timer > 2000)
    { // we got the preset we were after or time out, and need to ask for the next one
  ...
  if (spark_msg_in.get_message(&cmdsub, &msg, &preset) || app_msg_in.get_message(&cmdsub, &msg, &preset))
  {
    ...
    case 0x0201:
      ...
      else if (ui_update_in_progress == UI_HARDWARE)
      {
        ...
        if (p >= 3 || hw_preset_requested >= 4)
        { // last preset received
          ui_update_in_progress = UI_NONE;
